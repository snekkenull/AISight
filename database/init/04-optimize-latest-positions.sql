-- Optimization: Create a materialized table for latest vessel positions
-- This dramatically improves query performance for bounding box queries

-- Create a table to store only the latest position per vessel
-- This is much faster than using DISTINCT ON across all position_reports
...
-- Composite index for bounding box queries (covers lat/lon range scans)
CREATE INDEX IF NOT EXISTS idx_latest_pos_bbox ON vessel_latest_positions(latitude, longitude, mmsi);

-- Function to upsert latest position (called when new position arrives)
...
-- Trigger function to automatically update latest position when new position is inserted
...
-- Create trigger on position_reports to auto-update latest positions
...
-- Populate the table with existing data (one-time migration)
...
-- Update the latest_vessel_positions view to use the optimized table
